...
function ItinerarySearch() {
	const [state, setState] = useState<SearchData>({ query: ''})
	const api = useAPI()
	...
	function handleSubmit(e: any){
		e.preventDefault()		
		console.log('Requisição GET Roteiros');

		if (state.query){
			api.get('/roteiros', {}, {}).then((res) => {
				console.log(res)
			})
                    ...
                    <div className="w-100 rounded-4 bg-white p-5 mb-5">
                        <form id="query-form" onSubmit={handleSubmit}>
							<div className="input-group mb-3">
								...
								name="query" id="query" placeholder="Pesquisar roteiro por UF ou cidade"
								aria-label="Pesquisar roteiro"
								aria-describedby="button-addon2"
								value={state.query} 
								onChange={(e) => onUpdate(e, 'query')} 
								/>
								<button className="btn btn-outline-secondary btn-lg" type="submit" id="button-search"><i className="bi bi-search"></i></button>
							</div>
						</form>
                        <div id="filters">
